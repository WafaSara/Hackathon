...
$(function() {
	...
	$( "body" ).addClass('multilingue_calendar');
...
$(window).load(function() {
  $("#slider-comment .slider-comment-content").css('height', $("li.panel").height())
});

$('#pagination-comment li').click(function(){
	index=$('#pagination-comment li').index($(this));
	height=$('.slider-comment-content li').eq(index).height();
...
function loadDone(){
	...
	$("#selecteurChambres").change(function(){
		...
		var nombreActuel=$("#colonne .chambres").length;
		if(nombreDesire>=nombreActuel){
			...
			for(var i=nombreActuel;i>nombreDesire;i--){
				$("#colonne .chambres").eq(i-1).remove();
			}
		}
		if(nombreDesire>1){
...
function refreshKidSelectors(){
	var chbre=0;
	$.each($("#colonne .selecteurEnfants"), function(){
		chbre++;
...
expandText(540, 220, "#presentation");

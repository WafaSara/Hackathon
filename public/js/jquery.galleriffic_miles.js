	...
	$.fn.galleriffic = function(thumbsContainerSel, settings) {
		...
		$.extend(this, {
			...
			buildImage: function(image) {
				...
				/* Centrer la grande image */
				$widthVisuel=$(image).attr('width');
				if($widthVisuel>320) $widthVisuel=320;
				$parent=$('#gallery').parent().parent().parent();
				if($($parent).attr('class')=='fch_htl_right1')
				{
					$('#gallery').css('width',$widthVisuel);
					if($widthVisuel==320) $('#gallery').css('margin-left','0');
					else $('#gallery').css('margin','0 auto');
				}
			...
			rebuildThumbs: function() {
				...
				if (needsPagination && this.settings.enableBottomPager) {
					...
					this.$thumbsContainer.find('div.bottom.pagination').empty();
				}
			...
			buildPager: function(pager) {
				...
				if (this.currentPage > 0) {
					...
					pager.append('<a class="showPrev" rel="history" href="#'+this.data[prevPage].hash+'" title="'+this.settings.prevPageLinkText+'">&nbsp;</a>');
				}
/*
				// Page Index Links
...
*/
				...
				if (nextPage < this.data.length) {
					pager.append('<a class="showNext" rel="history" href="#'+this.data[nextPage].hash+'" title="'+this.settings.nextPageLinkText+'">&nbsp;</a>');
				}
